            ...
            if args.add_fk_info:
                ...
                for table_id, column_id in table_column_id_list:
                    if extra_column_info_in_one_db[table_id][column_id] != "":
                        ...
                        extra_column_info_in_one_db[table_id][column_id] += "[FK]"
            # add multiturn mark
            if args.multiturn and mode == "train":
                if data["turn_idx"] > 0:
                    assert len(pre_tabcol_labels_in_one_db) != 0
                    for tab_id, col_labs in enumerate(pre_tabcol_labels_in_one_db):
                        for col_id, lab in enumerate(col_labs):
                            if lab == 1:
                                if extra_column_info_in_one_db[tab_id][col_id] != "":
                                    extra_column_info_in_one_db[tab_id][col_id] += " , [SN]"
                                else:
                                    extra_column_info_in_one_db[tab_id][col_id] += "[SN]"
                    if args.multiturn_wtable:
                        for tab_id, lab in enumerate(pre_tab_labels_in_one_db):
                            if lab == 1:
                                table_names_in_one_db[tab_id] = table_names_in_one_db[tab_id] + " ( [SN] )"
